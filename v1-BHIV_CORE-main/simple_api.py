...
if __name__ == "__main__":
    ...
    if is_port_in_use(args.port):
        ...
        if alternative_port:
            ...
            logger.error(f"Could not find available port near {args.port}")
            print(f"\nERROR: Port {args.port} is in use and no alternatives found.")
            print("Please run 'clear-bhiv-ports.bat' to clear existing processes.")
            exit(1)
    ...
    try:
        ...
        if "address already in use" in str(e).lower():
            logger.error(f"Port {args.port} is still in use. Please run clear-bhiv-ports.bat first.")
        else:
