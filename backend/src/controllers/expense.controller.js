...
// @desc    Process receipt with BHIV AI
// @route   POST /api/v1/expenses/:id/process-receipt
// @access  Private
export const processReceiptWithAI = async (req, res) => {
  try {
    const expense = await expenseService.getExpenseById(req.params.id);
    ...
    if (!expense.receipts || expense.receipts.length === 0) {
      return res.status(400).json({
        success: false,
        message: 'No receipts found for this expense',
      });
    }
    
    // Process the first receipt with BHIV
    const receiptPath = expense.receipts[0].path;
    const analysis = await bhivService.analyzeExpenseReceipt(receiptPath);
    
    if (!analysis) {
      return res.status(503).json({
        success: false,
        message: 'BHIV AI processing unavailable',
      });
    }
    ...
    logger.error('Process receipt with AI error:', error);
    res.status(500).json({
      success: false,
      message: error.message,
    });
